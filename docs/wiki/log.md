...
## 2026-06-12 — census prose gains the symlink-bridge gotcha (writing through a pre-ADR-008 CLAUDE.md symlink clobbers AGENTS.md — hit live during the production-project heal); flow-refresh re-pointed the shifted update.md citations, no-op re-stamp overview
## 2026-06-12 — audit cost model corrected after the first production run (a production wiki, ~15x understated): byte-aware budget in references/audit.md Phase 0, §7 field correction in RESEARCH-AUDIT, dated Correction on adr-010; no-op re-stamp howto/audit-the-wiki
## 2026-06-12 — updated vendored tooling v0.4.6 → v0.4.7 (version stamp — cost-model correction is docs/procedure only, no script change); no-op re-stamp overview
## 2026-06-12 — no-op re-stamp gotchas/cross-tool-depth-untested (covers RESEARCH.md, which got a cosmetic reference scrub)
